...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="ss:Worksheet" mode="mods">
        ...
        <xsl:variable name="bmtnid" select="$journals//journal[name = $ssname]/id" as="xs:string"/>
    ...
    <xsl:template match="ss:Row" mode="mets">
        <xsl:if test="ss:Cell[1]/ss:Data">
            ...
            <xsl:variable name="issueno" select="ss:Cell[4]/ss:Data"/>
            <xsl:variable name="callnum" select="ss:Cell[5]/ss:Data"/>
            <xsl:variable name="date" select="ss:Cell[6]/ss:Data"/>
            <xsl:variable name="extent" select="ss:Cell[7]/ss:Data"/>
            <xsl:variable name="pcount" select="ss:Cell[8]/ss:Data"/>
            ...
            <xsl:variable name="basename-old">
                <xsl:value-of
                    select="concat('bmtn030-', format-number($volume,'00'), format-number($issueno,'00'))"
                />
            </xsl:variable>

            <xsl:variable name="basename">
                <xsl:value-of select="concat('bmtn030-', local:convertDate($date))"/>
            </xsl:variable>

            <xsl:variable name="filename">
                <xsl:value-of
                    select="concat('/tmp/bmtn030/issues/',$basename,'/',$basename,'.mets.xml' )"/>
            </xsl:variable>
    ...
    <xsl:template match="ss:Row" mode="mods">
        ...
        <xsl:if test="ss:Cell[1]/ss:Data">
            ...
            <xsl:variable name="isodate" select="local:convertDate($date)"/>
            ...
            <xsl:variable name="basename">
                <xsl:choose>
                    <xsl:when test="$isodate">
                        <xsl:value-of select="concat('bmtn030-', $isodate)"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:message terminate="yes">
                            <xsl:value-of select="concat($date, 'not a valid date.')"/>
                        </xsl:message>
                    </xsl:otherwise>
                </xsl:choose>
            ...
            <xsl:result-document href="{$filename}">
                <mods xmlns="http://www.loc.gov/mods/v3" xmlns:xlink="http://www.w3.org/1999/xlink">
                    ...
                    <relatedItem type="host" xlink:type="simple"
                        xlink:href="urn:PUL:bluemountain:bmtn030">
                        <recordInfo>
                            <recordIdentifier>urn:PUL:bluemountain:dmd:bmtn030</recordIdentifier>
                        </recordInfo>
                    </relatedItem>
                </mods>
            </xsl:result-document>
