...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="bmtnid" as="xs:string">bmtnaaf</xsl:variable>
        ...
        <xsl:if test="ss:Cell[1]/ss:Data">
	      ...
	      <xsl:value-of select="concat($bmtnid, '_', local:issueID($bmtnid, $keydate))"/>
		    ...
		    <titleInfo type="uniform">
		      <title>Elan (Paris, France)</title>
		    </titleInfo>
                    <titleInfo>
		      <xsl:choose>
			<xsl:when test="matches($title, concat('^[Ll]',$apos))">
			  <nonSort><xsl:value-of select="substring($title, 1, 2)"/></nonSort>
			  <title><xsl:value-of select="substring($title, 3)"/></title>
			</xsl:when>
			<xsl:otherwise>
                        <title>
